...
use canva_connect::{endpoints::assets::AssetUploadMetadata, models::*};
...
fn test_asset_upload_metadata_serialization() {
    ...
    assert_eq!(
        json.get("name_base64").unwrap().as_str().unwrap(),
...
fn test_asset_upload_metadata_with_tags() {
    let metadata =
        AssetUploadMetadata::new("test.png", vec!["tag1".to_string(), "tag2".to_string()]);
