...
impl AssetsApi {
    ...
    pub async fn create_upload_job(&self, file_data: Vec<u8>, metadata: AssetUploadMetadata) -> Result<Job<AssetUploadResult>> {
        let metadata_json = serde_json::to_string(&metadata)?;
        let response = self.client.upload_file("/v1/asset-uploads", file_data, Some(&metadata_json)).await?;
        let job: Job<AssetUploadResult> = response.json().await?;
        Ok(job)
    }

    /// Get the status of an asset upload job
    pub async fn get_upload_job(&self, job_id: &str) -> Result<Job<AssetUploadResult>> {
        let path = format!("/v1/asset-uploads/{}", job_id);
        self.client.get_json(&path).await
    }

    /// Create an asset upload job from URL
    pub async fn create_url_upload_job(&self, request: CreateUrlAssetUploadJobRequest) -> Result<Job<AssetUploadResult>> {
        let response: Job<AssetUploadResult> = self.client.post_json("/v1/url-asset-uploads", &request).await?;
        Ok(response)
    }

    /// Get the status of a URL asset upload job
    pub async fn get_url_upload_job(&self, job_id: &str) -> Result<Job<AssetUploadResult>> {
        let path = format!("/v1/url-asset-uploads/{}", job_id);
        self.client.get_json(&path).await
    }

    /// Wait for an upload job to complete
    pub async fn wait_for_upload_job(&self, job_id: &str) -> Result<AssetUploadResult> {
        loop {
            let job = self.get_upload_job(job_id).await?;
            
            match job.status {
                JobStatus::Success => {
                    return job.result.ok_or_else(|| crate::error::Error::Generic("Job succeeded but no result".to_string()));
                }
                JobStatus::Failed => {
    ...
    pub async fn wait_for_url_upload_job(&self, job_id: &str) -> Result<AssetUploadResult> {
        loop {
